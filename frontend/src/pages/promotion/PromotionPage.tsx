...
const PromotionPage: React.FC = () => {
    ...
    const [sortField, setSortField] = useState<keyof PromotionSchema | null>("name");
    ...
    const { data, isLoading } = useQuery<ResponsePagination<PromotionSchema>, Error>({
        queryKey: ["promotions"],
    ...
    return (
        <Box>
            ...
            <Stack direction="row" justifyContent="flex-end" sx={{ marginBottom: '16px', mr: 2 }}>
                <Button
                    variant="contained"
                    color="success"
            ...
            <TableContainer component={Paper} sx={{ margin: '16px', backgroundColor: 'white' }}>
                {isLoading ? (
                    <CircularProgress />
                ) : (
                    <Table>
                        <TableHead sx={{ backgroundColor: colors.secondaryColor }}>
                            <TableRow>
                                {["ID", "Tên", "Mô tả", "Ngày bắt đầu", "Ngày kết thúc", "Loại khuyến mãi", "Giới hạn đơn hàng", "Trạng thái"].map((header, index) => {
                                    const fieldNames = ["id", "name", "description", "startDate", "endDate", "typePromotion", "orderLimit", "status"];
                                    const field = fieldNames[index];
                                    return (
                                        <TableCell
                                            key={index}
                                            align="center"
                                            sx={{ padding: '16px', fontWeight: 'bold', cursor: 'pointer' }}
                                            onClick={() => {
                                                if (field) {
                        ...
                        <TableBody>
                            {promotions.map((promotion) => (
                                <TableRow key={promotion.id} hover>
                                    ...
                                    <TableCell align="center">{promotion.typePromotion}</TableCell>
                                    <TableCell align="center">{promotion.orderLimit}</TableCell>
                                    <TableCell align="center">{getPromotionStatus(promotion.startDate, promotion.endDate)}</TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                    </Table>
