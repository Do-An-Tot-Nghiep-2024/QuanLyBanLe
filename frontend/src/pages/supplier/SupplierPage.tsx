...
export default function SupplierPage() {
  ...
  const getSuppliers = async () => {
    ...
    // Sort suppliers
    if (sortField) {
      sortedSuppliers.sort((a: { [x: string]: number; }, b: { [x: string]: number; }) => {
  ...
  const columns: Array<{ label: string; field?: keyof Supplier; width?: string; maxWidth?: string }> = [
          ...
          <Table aria-label="custom pagination table">
            ...
            <TableBody>
              {suppliers.map((supplier) => (
                <TableRow hover key={supplier.id} sx={{ height: "60px", overflow: "hidden" }}>
                  <TableCell align={"center"} sx={styles.tableCell}>{supplier.name}</TableCell>
                  <TableCell align={"center"} sx={styles.tableCell}>{supplier.phone}</TableCell>
                  <TableCell align={"center"} sx={styles.tableCell}>{supplier.email}</TableCell>
